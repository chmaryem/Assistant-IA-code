...
from knowledge_loader import ProjectCodeIndexer
...
class SystemAwareRAG:
    """
    RAG conscient du système — 3 recherches ChromaDB au lieu d'1.

    Stratégie multi-query :
      Query 1 : code du fichier actuel          (pertinence directe)
      Query 2 : noms des méthodes predecessors  (règles sur les appelants)
      Query 3 : noms des méthodes successors    (règles sur les dépendances)

    Les résultats sont fusionnés, dédupliqués par source_file+chunk_index,
    re-triés par score et limités à top_k.

    Utilise directement le vector_store de assistant_agent (même ChromaDB).
    """
    ...
    def __init__(self, vector_store, language: str, project_code_indexer=None):
        ...
        self.project_code_indexer  = project_code_indexer   # Changement 1
    ...
    def retrieve(
        ...
        """
        Retourne (docs, scores) fusionnés depuis 4 recherches :
          1. code actuel          → KB de règles génériques
          2. signatures appelants → KB de règles sur les contrats
          3. signatures dépendances → KB de règles sur les interfaces
          4. code actuel          → project_code_index (code réel du projet)
             → trouve comment les autres fichiers gèrent les mêmes patterns
        """
        queries = self._build_queries(current_code, neighborhood)

        # Dictionnaire de déduplication : clé unique → (doc, meilleur_score)
        seen: Dict[str, Tuple[Any, float]] = {}

        # ── Recherches 1-3 dans la KB de règles génériques ────────────────────
        for query in queries:
            ...
            if results and query == queries[0]:
        ...
        # ── Recherche 4 dans project_code_index (Changement 1) ───────────────
        # Trouve comment les AUTRES fichiers du projet gèrent les mêmes patterns.
        # Ex : UserService a un resource leak → retrouve findByUsername() dans
        #      UserRepository qui a le même problème → LLM voit le problème en cascade.
        if self.project_code_indexer:
            project_results = self.project_code_indexer.search(
                query        = current_code[:600],
                k            = 4,
                exclude_file = current_file_name,  # évite l'autoréférence
        ...
        # Tri : KB rules en premier (prefix "kb:"), puis project code, puis par score
        ranked = sorted(
            seen.values(),
            key=lambda pair: (
                0 if pair[0].metadata.get("language", "") == self.language else 1,
                # KB rules avant project code (règles plus directement actionnables)
...
class IncrementalAnalyzer:
    ...
    def initialize(self):
        """Initialise le graphe, l'indexeur projet et le neighborhood extractor."""
        ...
        self._neighborhood_extractor = GraphNeighborhoodExtractor(
            ...
            project_indexer  = self.project_indexer,   # context.files + get_related_files()
        )

        # ProjectCodeIndexer — Changement 1
        # Réutilise les embeddings déjà chargés dans assistant_agent (pas de double chargement)
        self._project_code_indexer = ProjectCodeIndexer(
            embeddings = assistant_agent.embeddings,
        )
        # Indexation initiale du projet (skip si déjà peuplé)
        ...
        print(f" System-Aware RAG activé — voisinage NetworkX opérationnel\n")
    ...
    def _analyze_file(self, file_path: Path):
        ...
        # ── ÉTAPE 4.5 : Mise à jour ProjectCodeIndexer (Changement 1) ─────────
        # NON-BLOQUANT : timeout réel via shutdown(wait=False).
        # "with ThreadPoolExecutor" attend la fin du thread même après timeout →
        # on utilise shutdown(wait=False) pour forcer la sortie immédiate.
        if self._project_code_indexer:
            ...
            try:
                ...
                future.result(timeout=4)   # 4s max
                ...
                ex.shutdown(wait=False)    # ← ne pas attendre le thread bloqué
        ...
        system_rag = SystemAwareRAG(
            ...
            language             = language,
            project_code_indexer = self._project_code_indexer,   # Changement 1
        )
        relevant_docs, rag_scores = system_rag.retrieve(
            current_code      = new_content,
            neighborhood      = neighborhood,
            current_file_name = file_path.name,   # évite l'autoréférence
        )
        ...
        with self._print_lock:
            _print_results(
                ...
                impacted       = preds,  # predecessors = fichiers impactés
            )
